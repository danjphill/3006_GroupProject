...
{
//    INT8U err;
//    OSSemPend(Sem1, 0, &err); // Tries to Get Semaphore
//    TRISBbits.RB1 = 0;
//    SetDDRamAddr(0x00);
//    while( BusyXLCD());
...
//    OSSemPost(Sem1); //Returns Semaphore
    for(;;){
   TRISCbits.TRISC2 = 0;
    SetDCPWM1(30);
...
//      Delay1KTCYx(400*length[i]); //each note is played for 400ms*relative length
      ...
      Delay10KTCYx(100);   //the silence is played for 50 ms   
    }
}
...
//Sem1 = OSSemCreate(1); /* Create Display Semaphore */
